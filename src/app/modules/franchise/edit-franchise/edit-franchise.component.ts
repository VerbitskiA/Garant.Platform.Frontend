...
export class EditFranchiseComponent implements OnInit {
  ...
  private getTransitionAsync() {
    this.commonService.getTransitionAsync(this.routeParam)
      .subscribe((data: any) => this.getViewFranchiseAsync(this.franchiseId <= 0 || this.franchiseId == undefined ? this.route.snapshot.queryParams["franchiseId"] : this.franchiseId));
  };
  ...
  private  getViewFranchiseAsync(franchiseId: number) {
    this.http.get<Franchise.IItem>(API_URL.apiUrl.concat("/franchise/get-franchise?franchiseId=" + franchiseId))
      .subscribe( (response) => {
  ...
  public  onEditFranchiseAsync() {
    ...
    let investInJson = { Name: nameInvest,Price: priceInvest};
    ...
    ).subscribe(response =>  this.showMessageAfterSuccessEditFranchise(), (error) => {
